...
<div class="container">
    <center>
        ...
        <span id="mais">
            <form enctype="multipart/form-data" action="/perfil/update" method="POST" style="width:50%; margin-top:30px;">
                @csrf
    ...
    <table class="table" style="margin-top:50px;">
        <tbody>
            ...
            <tr>
                <th>Curso:</th>
                <td style="text-align:right;">{{$curso->curso}}</td>
            </tr>
        </tbody>
    </table>
    <form method="POST" action="{{ route('perfil/apagar') }}" onsubmit="if(!confirm('Tem certeza que deseja apagar sua conta?')){ return false; }">
        @csrf
        <input type="hidden" id="{{ Auth::user()->id }}" name="user_id" value="{{ Auth::user()->id }}">
    ...
    <a href="/perfil/editar/{{Auth::user()->id}}" class="btn" style="color:white;font-weight:medium;background-color: #1C40C3;border-radius: 20px;position: absolute;right:10%;">Editar</a>
</div>
<script>
    function leiaMais() {
